...
public class DAOCarImpl implements DAOCar {
    ...
    private static final String  REQUEST_FOR_INSERT="INSERT INTO `park`.`car` " +
            "(`transport_type`, `driverId`, `regNumber`,`status`,`available`,`booking_date`) VALUES(?,?,?,?,?,?)";
    ...
    private static final String UPDATE_STATUS="UPDATE `park`.`car` SET `status` = ? WHERE id =?";
    ...
    @Override
    public void changeStatus(String status, int id) throws DAOException {
        Connection con=null;
    ...
    public Car findCar(int id) throws DAOException {
        ...
        try {
            ...
            while(rs.next()){
                ...
                car.setBookingDate(rs.getString("booking_date"));
                car.setId(id);
            }
    ...
    public void addCar(Car car) throws DAOException{
        ...
        try {
            ...
            pstm.setString(4, car.getStatus());
            pstm.setString(5,car.getAvailable());
            pstm.setString(6,car.getBookingDate());
            pstm.executeUpdate();
